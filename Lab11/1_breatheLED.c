...
void initKB(){
    LPC_GPIO0->FIODIR |= 0x0F0;    //P0.8 to P0.11 rows (o/p)
    LPC_GPIO0->FIODIR &= ~(0x0F00);   //P0.4 to P0.7 as cols (i/p)
}

int scan(){
		int pressed;
		LPC_GPIO0->FIOSET = (0x1<<4);	//set row 1 
		pressed = LPC_GPIO0->FIOPIN;
		pressed &= 0xF00;		//mask and check which key is pressed
		pressed >>= 8;			//shift to LSB
		return pressed;
}
